CREATE OR REPLACE FUNCTION public.CronJob_Disable(
_SchemaName   text,
_FunctionName text
)
RETURNS integer
...
ELSE
    IF _Enabled IS TRUE THEN
        UPDATE CronJobs SET Enabled = FALSE WHERE CronJobID = _CronJobID AND Enabled IS TRUE RETURNING TRUE INTO STRICT _OK;
    ELSIF _Enabled IS FALSE THEN
        RAISE NOTICE 'Function %.% with CronJobID % has already been disabled', _CronJobID, _SchemaName, _FunctionName;
    ELSE
