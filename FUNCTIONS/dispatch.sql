...
SELECT
    ...
    extract(epoch from CASE
        ...
        WHEN J.RunUntilTime      > now()::time THEN NULL
...
RAISE DEBUG '% pg_backend_pid % : spawn new process -> [JobID % Function % RunProcessID % RunInSeconds % MaxProcesses %]', clock_timestamp()::timestamp(3), pg_backend_pid(), _JobID, _Function, RunProcessID, RunInSeconds, MaxProcesses;
RETURN;
