CREATE OR REPLACE FUNCTION cron.Dispatch(OUT RunProcessID integer, OUT RunInSeconds numeric, OUT LimitProcesses integer)
RETURNS RECORD
LANGUAGE plpgsql
...
WHERE NOT P.Running
AND (cron.No_Waiting()                       OR J.RunIfWaiting = TRUE)
...
RAISE DEBUG '% pg_backend_pid % : spawn new process -> [JobID % Function % RunProcessID % RunInSeconds % LimitProcesses %]', clock_timestamp()::timestamp(3), pg_backend_pid(), _JobID, _Function, RunProcessID, RunInSeconds, LimitProcesses;
RETURN;
